  ...
  return (
    <section id="pricing" className="py-16 md:py-24 px-4 bg-white">
      <div className="max-w-6xl mx-auto text-center">
        ...
        {/* Plan Cards */}
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-16" staggerDelay={0.12}>
          {PLANS.map((plan, index) => {
            ...
            return (
              <StaggerItem key={plan}>
                <TiltCard className={`relative h-full ${isPopular ? 'animated-border rounded-2xl' : ''}`}>
                  <div
                    className={`relative bg-[#F7F7F5] border rounded-2xl p-6 text-left h-full transition-shadow hover:shadow-lg ${
                      isPopular
                  ...
                  >
                    ...
                    <button
                      className={`w-full py-3 rounded-full font-medium text-sm transition-all min-h-[44px] ${
                        isPopular
                          ? 'shimmer-hover bg-[#FF6B35] text-white hover:bg-[#E55A2B] hover:shadow-md'
